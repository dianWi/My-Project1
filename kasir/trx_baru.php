...
$query = mysqli_query($conn,$sql);
	...
	$sqltrx="INSERT INTO trx(id_trx,id_mek,tgl_trx,total,id_kasir)
		  VALUES('$no','$mek','$tg','$grand','$kasir')";
						...
						<form class="form-horizontal" method="POST" enctype="multipart/form-data">
							<div class="panel-body">
									<div class="form-group">
										<label class="control-label col-sm-3">Mekanik</label>
										<div class="col-sm-4">
											<select name="mek" id="mek" class="form-control" required>
												<option value="">==== Pilih Mekanik ====</option>
												<option value="0">Umum</option>
												<?php
													$sql_brg = "SELECT * FROM mekanik WHERE id_mek!='0' ORDER BY nama_mek ASC";
													$ress_brg = mysqli_query($conn, $sql_brg);
													while($li = mysqli_fetch_array($ress_brg)) {
														echo '<option value="'. $li['id_mek'] .'">'. $li['nama_mek'].'</option>';
													}
												?>
											</select>
										</div>
